...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare(
        "SELECT * FROM counsellors WHERE counsellor_id = ? AND contact_info = ?"
    );
    $stmt->bind_param('is', $counsellor_id, $contact_info);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 1) {
        $row = $result->fetch_assoc();
        $_SESSION['counsellor_id'] = $row['counsellor_id'];
        header('Location: counsellorDashbaord.php');
        exit();
    } else {
